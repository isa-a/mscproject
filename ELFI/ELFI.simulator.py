...
observeddata = pd.read_csv('file.csv')
...
def tauWrapper(t, X, N, K, r, alpha1, alpha2, tau, measurement_times,
    ...
    measured_pop = np.asanyarray(pops[measurement_times, :])
...
def logdestack(final_pop): #reshaping the output
    x = np.array(final_pop).flatten().reshape(1, -1) #flatten turns it into one dimension (1 column) and then reshape makes that into a row so it can be used later on
    return(np.log(np.fmax(x, np.zeros(x.shape)) + 1)) #taking logs and adding 1 as log0 not possible. fmax ensures any possible negative values are turned into 0. arrays cause errors
    #TypeError In executing node '__summary_63e5_observed': '>=' not supported between instances of 'str' and 'float



alphaprior1 = elfi.Prior(ss.uniform, 0, 5)
alphaprior2 = elfi.Prior(ss.uniform, 0, 5)#defining priors for alpha 

vectorized_sim = elfi.tools.vectorize(tauWrapper, [0, 1, 2, 3, 4, 7, 8])#selecting paramters from list to stack upon one another
